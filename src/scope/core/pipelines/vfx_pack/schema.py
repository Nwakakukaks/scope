...
class VFXConfig(BasePipelineConfig):
    ...
    halftone_enabled: bool = Field(
        default=False,
        description="Enable halftone effect (newspaper dot pattern)",
        json_schema_extra=ui_field_config(order=20, label="Halftone"),
    )
    ...
    halftone_dot_size: int = Field(
        ...
        ge=4,
        le=20,
        description="Size of halftone dots in pixels (larger = more stylised)",
        json_schema_extra=ui_field_config(order=21, label="Dot Size"),
    )
    ...
    halftone_sharpness: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Edge sharpness of the dots (0 = soft, 1 = hard)",
        json_schema_extra=ui_field_config(order=22, label="Sharpness"),
    )
